...
class SignupForm(forms.ModelForm):
    ...
    confirm_password = forms.CharField(widget=forms.PasswordInput(
        attrs={'placeholder': 'confirm_password', 'class': 'form-control'}))
    blood_group = forms.CharField(widget=forms.Select(
        choices=blood_type, attrs={'class': 'custom-select'}))

    class Meta:
        model = Account
        fields = ['email', 'phone', 'blood_group',
                  'address', 'username', 'last_date_of_donation']
        widgets = {
            ...
            'last_date_of_donation': DatePickerInput(format='%m/%d/%Y'),
        }
...
class Password_verification_form(forms.Form):
    token = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Enter Your Token', 'class': 'form-control'}))
...
class ProfleUpdateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['username', 'email', 'phone', 'address',
                  'blood_group', 'last_date_of_donation', 'image']
        widgets = {
    ...
    def clean(self):
        ...
        duplicate_email = Account.objects.filter(
            email=email).exclude(id=self.user.id)
        if duplicate_email.exists():
            raise forms.ValidationError('This email is already registered')

        duplicate_phone = Account.objects.filter(
            phone=phone).exclude(id=self.user.id)
